...
#define EXTRASTARTBYTES 10
// Still, glitches do occasionally happen.  Workarounds for now include
// frequent refreshes (so glitch bits don't stick around), make sure
// connections AND GROUNDS are super clean.

static const    uint8_t  defaultPins[] = { 0,1,2,3,4,5,6,7 };
...
boolean Adafruit_NeoPXL8::begin(void) {
  ...
  if(pixels) { // Successful malloc of NeoPixel buffer?
    ...
    if((dmaBuf = (uint8_t *)malloc(bytesTotal))) {
      ...
      // First few bytes are zero-filled
      memset(startAddr, 0, EXTRASTARTBYTES);
